...
export default function OnboardingPage() {
  ...
  // Wait for user to be fully authenticated and check if onboarding already done
  useEffect(() => {
    async function checkAuth() {
      if (!supabaseClient) return
      const { data: { session } } = await supabaseClient.auth.getSession()
      if (session?.user) {
        // Check if onboarding is already completed (proxy: name and goal set)
        ...
        if (profile?.name && profile.name.length > 0 && profile.goal) {
          router.push('/')
          return
        }
        setUserReady(true)
  ...
  const onSubmit = async (data: OnboardingForm) => {
    ...
    try {
      ...
      // STEP 1: Upsert user profile (mark onboarding as completed)
