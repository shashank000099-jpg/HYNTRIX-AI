...
const SUPABASE_REDIRECT_URL = 'https://hyntrixai.com/auth/callback.html';
const SUPABASE_RESET_REDIRECT = 'https://hyntrixai.com/reset-password.html';

// For development/localhost
if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
  const protocol = window.location.protocol;
  const host = window.location.host;
  window.SUPABASE_REDIRECT_URL = `${protocol}//${host}/auth/callback.html`;
  window.SUPABASE_RESET_REDIRECT = `${protocol}//${host}/reset-password.html`;
}
...
async function resendConfirmationEmail(email) {
  const data = await authFetch('/auth/v1/otp', 'POST', {
    email,
    type: 'signup',
    redirect_to: window.SUPABASE_REDIRECT_URL || SUPABASE_REDIRECT_URL
  });
